...
var JFUtil = {
    ...
    /**
     *
     * @returns {{buildView: buildView}}
     * @constructor
     * @desc    生成多级列表（参考实例demo3.html）
     */
    XListModel : function () {
        var resultStr = '';
        var completeFlag = false;
        var buildView = function (dataItem) {
            if (dataItem) {
                resultStr += "<li><header>" + dataItem.name + "</header><ul>";
                if (!dataItem.children || dataItem.children.length == 0) {
                    //已经到底了
                    completeFlag = true;
                    resultStr += "</ul></li>";
                } else {
                    for (var child of dataItem.children) {
                        buildView(child);
                    }
                    resultStr += "</ul></li>";
                }
